...
function CategorySlugContent({ categorySlug }: { categorySlug: string }) {
  ...
  const clearFilters = () => {
    setFilters({ minPrice: "", maxPrice: "", minRating: null, inStock: false });
    ...
    <div className="space-y-6">
      {onClose && (
        <div className="flex items-center justify-between pb-4 border-b mb-4 lg:hidden">
          ...
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-lg" aria-label="Close filters">
            <X className="w-5 h-5" />
          </button>
        </div>
      )}
      ...
      <div>
        ...
        <nav aria-label="Product categories" className="space-y-1">
          <Link
            href="/category"
            onClick={onClose}
            className="flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors text-gray-600 hover:bg-gray-50"
          >
    ...
    <div className="min-h-screen bg-white">
      <div className="border-b">
        <div className="container py-6">
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm text-gray-400 mb-3">
            ...
            <div className="sticky top-24">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
                  ...
                  Filters
                  {activeFilterCount > 0 && (
                    <span className="bg-gray-900 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center" aria-label={`${activeFilterCount} active filters`}>
                      {activeFilterCount}
                    </span>
                  )}
                </h3>
                <button onClick={() => setFiltersCollapsed(true)} className="p-1 text-gray-400 hover:text-gray-600 rounded" aria-label="Collapse filters">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <FilterPanel />
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-4 mb-6 pb-4 border-b">
              ...
              <div className="flex items-center gap-3">
                <div className="hidden sm:flex items-center border rounded-lg" role="group" aria-label="Grid size">
                  <button onClick={() => setGridSize("large")} className={`p-2 ${gridSize === "large" ? "bg-gray-100" : ""}`} aria-label="Large grid" aria-pressed={gridSize === "large"}>
                    <LayoutGrid className="w-4 h-4" />
                  </button>
                  <button onClick={() => setGridSize("small")} className={`p-2 ${gridSize === "small" ? "bg-gray-100" : ""}`} aria-label="Small grid" aria-pressed={gridSize === "small"}>
              ...
              <div className="flex flex-wrap items-center gap-2 mb-4" aria-live="polite">
                ...
                {(filters.minPrice || filters.maxPrice) && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 text-xs bg-gray-100 rounded-full">
                    ...
                    <button onClick={() => { updateFilters({ minPrice: "", maxPrice: "" }); setPendingPrice({ min: "", max: "" }); }} className="hover:text-red-500" aria-label="Remove price filter">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                )}
                {filters.minRating && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 text-xs bg-yellow-50 rounded-full">
                    ...
                    <button onClick={() => updateFilters({ minRating: null })} className="hover:text-red-500" aria-label="Remove rating filter">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                )}
                {filters.inStock && (
                  <span className="inline-flex items-center gap-1 px-2 py-1 text-xs bg-green-50 rounded-full">
                    In Stock
                    <button onClick={() => updateFilters({ inStock: false })} className="hover:text-red-500" aria-label="Remove stock filter">
                      <X className="w-3 h-3" />
                    </button>
                  </span>
                )}
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <button onClick={() => { setPage(1); loadProducts(); }} className="text-sm underline">Try again</button>
              </div>
            ) : products.length > 0 ? (
              <div className={`grid gap-4 ${gridSize === "small" ? "grid-cols-2 sm:grid-cols-3 xl:grid-cols-4" : "grid-cols-2 lg:grid-cols-3"}`}>
              ...
              <div className="text-center py-16">
                ...
                {activeFilterCount > 0 ? (
                  ...
                  <Link href="/category" className="text-sm underline">View all products</Link>
                )}
              </div>
            )}
...
export default function CategorySlugPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <Suspense fallback={
      ...
      <CategorySlugContent categorySlug={slug} />
    </Suspense>
  );
}
