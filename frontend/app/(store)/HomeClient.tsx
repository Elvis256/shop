...
export default function HomeClient({
  ...
  const [trending] = useState<Product[]>(initialTrending);
  const [topRated] = useState<Product[]>(initialTopRated);
      ...
      <Section title="" bgColor="gray">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            <Link key={article.slug} href={`/blog/${article.slug}`} className="bg-surface rounded-xl p-6 hover:shadow-md transition-shadow group">
              <span className="text-xs font-medium text-primary bg-primary/10 px-2 py-1 rounded-full">{article.category}</span>
              <h3 className="mt-3 font-semibold text-text group-hover:text-primary transition-colors">{article.title}</h3>
