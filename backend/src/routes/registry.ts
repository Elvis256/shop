...
router.post("/", authenticate, asyncHandler(async (req: AuthRequest, res: Response) => {
  try {
    ...
    logger.error("Create registry error", { error });
...
router.post("/:id/items", authenticate, asyncHandler(async (req: AuthRequest, res: Response) => {
  try {
    const data = z.object({
      productId: z.string(),
...
router.delete("/:id/items/:itemId", authenticate, asyncHandler(async (req: AuthRequest, res: Response) => {
  try {
    const registry = await prisma.registry.findUnique({ where: { id: req.params.id } });
    if (!registry) return res.status(404).json({ error: "Registry not found" });
    if (registry.userId !== req.user!.id) return res.status(403).json({ error: "Not your registry" });
...
router.post("/:slug/purchased/:itemId", asyncHandler(async (req, res: Response) => {
  try {
